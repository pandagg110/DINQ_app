...
class AchievementNetworkCardDefinition extends CardDefinition {
  ...
  Map<String, dynamic>? adapt(dynamic rawMetadata) {
    ...
    if (alreadyAdapted['connections'] is List) {
      return {'connections': alreadyAdapted['connections']};
    }
    ...
    final topConnections = connectionsList.take(6).map((person) {
      final personMap = person as Map<String, dynamic>;
      return {
        'name': personMap['name'] ?? '',
        'avatarUrl':
            personMap['avatarUrl'] ??
            personMap['avatar_url'] ??
            '/images/default-avatar.svg',
        'institution_logo_url': personMap['institution_logo_url'],
        'affiliation': personMap['affiliation'] ?? '',
        'position': personMap['position'] ?? '',
        'relationshipType':
            personMap['relationshipType'] ??
            personMap['relationship_type'] ??
            '',
        'score': personMap['score'] ?? personMap['final_score'] ?? 0,
        'reason':
            personMap['reason'] ?? personMap['reason_for_inclusion'] ?? '',
      };
    }).toList();

    return {'connections': topConnections};
  }

  @override
  Widget render(CardRenderParams params) {
    return NetworkWidget(card: params.card, size: params.size);
  }
}
