...
class LinkedInComponents {
  ...
  /// Vertical Timeline for 2x4 layout - matches TSX render.tsx 2x4
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (yearSpan == 0 || careerJourney.length == 1) {
          ...
          rawPositions = careerJourney.map((item) {
        ...
        // Adjust positions for minimum spacing (48px between icon centers)
        const minGapPx = 48.0;
        final minGapPercent = minGapPx / height;

        final adjustedPositions = [...rawPositions];
        for (int i = 1; i < adjustedPositions.length; i++) {
          final prevPos = adjustedPositions[i - 1];
          final currentPos = adjustedPositions[i];
          final gap = currentPos - prevPos;

          if (gap < minGapPercent) {
            adjustedPositions[i] = prevPos + minGapPercent;
          }
        }

        // Normalize to fit within safe margins
        // Top/bottom margins: half logo size (so logo doesn't clip)
        final topMargin = halfLogo / height;
        final bottomMargin = halfLogo / height;
        final usableRange = 1.0 - topMargin - bottomMargin;

        final minPos = adjustedPositions.reduce((a, b) => a < b ? a : b);
        final maxPos = adjustedPositions.reduce((a, b) => a > b ? a : b);
        final posRange = (maxPos - minPos) != 0 ? (maxPos - minPos) : 1.0;

        // Convert to pixel positions (center of logo)
        final finalPositions = adjustedPositions.map((pos) {
          ...
          return (topMargin + normalized * usableRange) * height;
        }).toList();
          ...
          child: Stack(
            ...
            children: [
              ...
              ...careerJourney.asMap().entries.map((entry) {
                final index = entry.key;
                final item = entry.value;
                final top = finalPositions[index] - halfLogo;
                return Positioned(
                  left: 0,
                  top: top.clamp(0, height - logoSize),
                  child: Row(
  ...
  /// Horizontal Timeline for 4x2 layout - matches TSX render.tsx 4x2 structure
  static Widget buildHorizontalTimeline({
    ...
    // Calculate positions (same logic as TSX)
    final years = careerJourney.map((item) => item['year'] as int).toList();
    final minYear = years.reduce((a, b) => a < b ? a : b);
    final maxYear = years.reduce((a, b) => a > b ? a : b);
    final yearSpan = maxYear - minYear;
    ...
    // Enforce minimum 72px between icon centers (TSX)
    const estimatedContainerWidth = 600.0;
    const minGapPx = 72.0;
    final minGapPercent = (minGapPx / estimatedContainerWidth) * 100;
    ...
    final finalPositionsPercent = adjustedPositions.map((pos) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return Stack(
          clipBehavior: Clip.none,
          children: [
            // Timeline Line - behind icons, at icon center (16px from top for 32px icon)
            Positioned(
            ...
            ...careerJourney.asMap().entries.map((entry) {
              final index = entry.key;
              final item = entry.value;
              final centerX = (finalPositionsPercent[index] / 100) * width;
              final left = centerX - (logoSize / 2);
