...
class ScholarWidget extends StatelessWidget {
  const ScholarWidget({super.key, required this.card, required this.size});
  ...
  Widget build(BuildContext context) {
    final topTierPapers =
        (card.data.metadata['topTierPapers'] as num?)?.toInt() ?? 0;
    final totalCitations =
        (card.data.metadata['totalCitations'] as num?)?.toInt() ?? 0;
    final firstAuthorCitations =
        (card.data.metadata['firstAuthorCitations'] as num?)?.toInt() ?? 0;
    final hIndex = (card.data.metadata['hIndex'] as num?)?.toInt() ?? 0;
    final summary = (card.data.metadata['summary'] as String?) ?? '';

    switch (size) {
      case '2x2':
        return ScholarLayouts.build2x2Layout(totalCitations: totalCitations);
      case '2x4':
