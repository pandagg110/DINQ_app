...
/// My → Resume 简历管理。接 /resumes（list/create/delete）+ OSS 上传。
...
class _ResumePageState extends State<ResumePage> {
  ...
  Future<void> _upload() async {
    try {
      ...
      await _service.createResume(title: title, sourceUrl: url, fileName: picked.name);
      if (!mounted) return;
  ...
  Widget _body() {
    ...
    if (_resumes.isEmpty) {
      return const Center(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(Icons.description_outlined, size: 40, color: DinqTokens.textTertiary),
            SizedBox(height: 12),
            Text('No resume yet',
                style: TextStyle(
                    fontSize: 16, fontWeight: FontWeight.w600, color: DinqTokens.textPrimary)),
            SizedBox(height: 4),
            Text('Upload a PDF to power your DINQ Page.',
    ...
    return ListView(
      ...
      children: [for (final r in _resumes) _resumeRow(Map<String, dynamic>.from(r as Map))],
    );
  }
  ...
  Widget _resumeRow(Map<String, dynamic> r) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.picture_as_pdf_outlined, size: 26, color: Color(0xFFC27C5A)),
          ...
          if (sourceUrl.isNotEmpty)
            GestureDetector(
              ...
              child: const Padding(
                ...
                child: Icon(Icons.open_in_new_rounded, size: 20, color: DinqTokens.textSecondary),
              ),
            ),
          GestureDetector(
            behavior: HitTestBehavior.opaque,
